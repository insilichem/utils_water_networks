...
for pdb_file in os.listdir(pdb_folder):
    if pdb_file.endswith(".pdb"):
        ...
        atom_selection_chi1 = "(resid 491 and name CA) or (resid 491 and name CB) or (resid 491 and name CG) or (resid 491 and name CD)"
        atom_selection_chi2 = "(resid 491 and name C) or (resid 491 and name CA) or (resid 491 and name CB) or (resid 491 and name CG)"
        atom_selection_chi3 = "(resid 491 and name CB) or (resid 491 and name CG) or (resid 491 and name CD) or (resid 491 and name OE1)"
      
        
        # Get the selected atoms
        chi1_atoms = u.select_atoms(atom_selection_chi1)
        chi2_atoms = u.select_atoms(atom_selection_chi2)
        chi3_atoms = u.select_atoms(atom_selection_chi3)        

        
        # Get positions of the selected atoms
        chi1_positions = chi1_atoms.positions
        chi2_positions = chi2_atoms.positions
        chi3_positions = chi3_atoms.positions        
        ...
        chi3_dihedral_angle = np.rad2deg(mda.lib.distances.calc_dihedrals(chi3_positions[:-3], chi3_positions[1:-2], chi3_positions[2:-1], chi3_positions[3:])) 


        

        # Append the results to the list
        results.append((pdb_file, chi1_dihedral_angle[0], chi2_dihedral_angle[0], chi3_dihedral_angle[0]))

# Save the results to a text file
output_file = "dihedral_angles_results-GLU491-SINGLE.txt"
